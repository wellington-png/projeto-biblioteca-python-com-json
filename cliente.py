...
def cadastar_cliente(data, matricula, nome, email, telefone, quant_possivel=3):
    if is_existe_cliente(data, matricula):
        ...
        return data
    data['cliente'].append({'id': len(data['cliente']) + 1, 'matricula': matricula, 'nome': nome, 'email': email, 'telefone': telefone, 'is_active': True, 'quant_possivel': quant_possivel})
    write_json('data.json', data)
    return data['cliente'][-1]

def atualizar_cliente(data, matricula, nome=None, email=None, telefone=None, is_active=True, quant_possivel=None):
    for i in data['cliente']:
        if i['matricula'] == matricula and i['is_active'] == True:
            ...
            i['quant_possivel'] = quant_possivel if quant_possivel else i['quant_possivel']
            write_json('data.json', data)
...
if __name__ == '__main__':
    ...
    cadastar_cliente(get_json('data.json'), 'xxx', '852147', 'tes@email.com', '8555555', quant_possivel=3)